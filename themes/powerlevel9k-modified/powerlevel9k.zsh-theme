...
prompt_dir() {
  ...
  local current_icon=''
  if [[ $(print -P "%~") == '~' ]]; then
    "$1_prompt_segment" "$0_HOME" "$2" "blue" "white" "$current_path" 'HOME_ICON'
  elif [[ $(print -P "%~") == '~'* ]]; then
    "$1_prompt_segment" "$0_HOME_SUBFOLDER" "$2" "blue" "white" "$current_path" 'HOME_SUB_ICON'
  else
    "$1_prompt_segment" "$0_DEFAULT" "$2" "blue" "white" "$current_path" 'FOLDER_ICON'
  fi
}
